...
export declare type IsTuple<T> = T extends any[]
  ? any[] extends T
    ? never
    : T
  : never;
...
export declare type IsUnknown<T> = IsAny<T> extends true
  ? false
  : unknown extends T
  ? true
  : false;
...
export declare type RequiredKeys<T> = T extends unknown
  ? Exclude<keyof T, OptionalKeys<T>>
  : never;
/** Gets keys of properties of given type in object type */
export declare type PickKeys<T, P> = Exclude<
  keyof PickProperties<T, P>,
  undefined
>;
...
>;
export declare type NonEmptyObject<T extends {}> = keyof T extends never
  ? never
  : T;
...
declare type _MergeN<T extends readonly any[], Result> = T extends readonly [
  infer Head,
  ...infer Tail
]
...
export declare type MarkRequired<T, RK extends keyof T> = Exclude<T, RK> &
  Required<Pick<T, RK>>;
...
export declare type UnionToIntersection<U> = (
  ...
  ? I
  : never;
declare type StringLiteral<T> = T extends string
  ? string extends T
    ? never
    : T
  : never;
...
export declare type ElementOf<T extends readonly any[]> =
...
export declare type AsyncOrSyncType<T> = T extends AsyncOrSync<infer PT>
  ? PT
  : never;
export interface Newable<T> {
  new (...args: any[]): T;
}
declare type IsEqualConsideringWritability<X, Y> = (<T>() => T extends X
  ? 1
...
export declare type Head<T extends AnyArray> = T['length'] extends 0
  ? never
  : T[0];
